# -*- coding: utf-8 -*-
"""Compare LLM Outputs.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/18yXmGfU7ti3xlM73jy1OTeZCuyQnmMEq
...
df_clean = pd.read_excel("(Clean) 835 US Queries with LLM Responses (v2).xlsx")
...
# Filter rows where the USHMM word count is within 200 words of all three LLM responses
filtered_df = df[
    ((df[ushmm_col] - df[gpt_col]).abs() <= 200) &
    ((df[ushmm_col] - df[gemini_col]).abs() <= 200) &
    ((df[ushmm_col] - df[grok_col]).abs() <= 200)
]
